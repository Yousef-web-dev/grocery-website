import Butter from "../../assets/butter.webp";
import Strawberry from "../../assets/strawberry.webp";
import Eggs from "../../assets/eggs.webp";
import Cabbage from "../../assets/cabbage.webp";
import Eggplant from "../../assets/eggplant.webp";
import Shrimp from "../../assets/shrimp.webp";
import Kiwi from "../../assets/kiwi.webp";
import Capsicum from "../../assets/capsicum.webp";
import Broccoli from "../../assets/broccoli.webp";
import Yogurt from "../../assets/yogurt.webp";
import Beef from "../../assets/beef.webp";
import ChickenBreast from "../../assets/chicken-breast.webp";
import Kale from "../../assets/kale.webp";
import Cheese from "../../assets/cheese.webp";
import CondensedMilk from "../../assets/condensed-milk.webp";
import Salmon from "../../assets/salmon.webp";
import Banana from "../../assets/banana.webp";
import Milk from "../../assets/milk.webp";
import Pineapple from "../../assets/pineapple.webp";
import Tofu from "../../assets/tofu.webp";
import Tilapia from "../../assets/tilapia.webp";
import SliceCheese from "../../assets/Slice-Cheese.webp";
import RicottaCheese from "../../assets/ricotta-Cheese.webp";
import Grapes from "../../assets/grapes.webp";
import Lettuce from "../../assets/lettuce.webp";

export const products = [
  {
    id: 1,
    name: "Strawberry",
    price: 3.0,
    category: "Fruits",
    image: Strawberry,
  },
  { id: 2, name: "Butter", price: 3.5, category: "Dairy", image: Butter },
  { id: 3, name: "Eggs", price: 2.5, category: "Dairy", image: Eggs },
  {
    id: 4,
    name: "Cabbage",
    price: 1.8,
    category: "Vegetables",
    image: Cabbage,
  },
  {
    id: 5,
    name: "Eggplant",
    price: 2.0,
    category: "Vegetables",
    image: Eggplant,
  },
  { id: 6, name: "Shrimp", price: 12.0, category: "SeaFood", image: Shrimp },
  { id: 7, name: "Kiwi", price: 4.0, category: "Fruits", image: Kiwi },
  {
    id: 8,
    name: "Capsicum",
    price: 2.2,
    category: "Vegetables",
    image: Capsicum,
  },
  {
    id: 9,
    name: "Broccoli",
    price: 2.8,
    category: "Vegetables",
    image: Broccoli,
  },
  { id: 10, name: "Yogurt", price: 1.5, category: "Dairy", image: Yogurt },
  { id: 11, name: "Beef", price: 15.0, category: "Meat", image: Beef },
  {
    id: 12,
    name: "Chicken Breast",
    price: 8.5,
    category: "Meat",
    image: ChickenBreast,
  },
  { id: 13, name: "Kale", price: 2.0, category: "Vegetables", image: Kale },
  { id: 14, name: "Cheese", price: 4.5, category: "Dairy", image: Cheese },
  {
    id: 15,
    name: "Condensed Milk",
    price: 3.2,
    category: "Dairy",
    image: CondensedMilk,
  },
  { id: 16, name: "Salmon", price: 14.0, category: "SeaFood", image: Salmon },
  { id: 17, name: "Banana", price: 1.5, category: "Fruits", image: Banana },
  { id: 18, name: "Milk", price: 2.0, category: "Dairy", image: Milk },
  {
    id: 19,
    name: "Pineapple",
    price: 5.0,
    category: "Fruits",
    image: Pineapple,
  },
  { id: 20, name: "Tofu", price: 3.0, category: "Vegetables", image: Tofu },
  { id: 21, name: "Tilapia", price: 9.0, category: "SeaFood", image: Tilapia },
  {
    id: 22,
    name: "Slice Cheese",
    price: 3.8,
    category: "Dairy",
    image: SliceCheese,
  },
  {
    id: 23,
    name: "Ricotta Cheese",
    price: 5.2,
    category: "Dairy",
    image: RicottaCheese,
  },
  { id: 24, name: "Grapes", price: 3.2, category: "Fruits", image: Grapes },
  {
    id: 25,
    name: "Lettuce",
    price: 1.5,
    category: "Vegetables",
    image: Lettuce,
  },
];
